...
/// Single-sample audio filter. Every filter type in this module implements this,
...
mod tests {
    ...
    #[test]
    fn biquad_identity_passes_sample() {
        let mut b = Biquad::new();
        b.set_identity();
        assert!((b.process(0.5) - 0.5).abs() < 1e-6);
    }

    #[test]
    fn clipper_hard_limits() {
        let y = MasteringClipper::process(2.0, 1.0, 0.0);
        assert!((y - 1.0).abs() < 1e-6);
    }

    #[test]
    fn peak_meter_tracks_abs() {
        let mut m = PeakMeter::new();
        m.feed(&[0.0, 0.5], &[0.25, -0.75]);
        assert!((m.peak_db() - 20.0 * 0.75f32.log10()).abs() < 1e-4);
    }

    #[test]
    fn state_migration_rejects_empty() {
        assert!(state_migration::try_parse_niceplug_state(&[]).is_none());
    }
}
