...
impl RelayHub {
    ...
    ///
    /// ```ignore
    /// if let Some((slot, generation)) = hub.claim_slot(aura_shm::now_ms()) {
    ...
    ///
    /// ```ignore
    /// if let Some(slot) = hub.claim_consumer_slot(aura_shm::now_ms()) {
    ...
    ///
    /// ```ignore
    /// let mut name_buf = [0u8; aura_shm::MAX_NAME_LEN];
    /// if let Some(len) = hub.single_consumer_name(aura_shm::now_ms(), &mut name_buf) {
    ...
    ///
    /// ```ignore
    /// let targets = hub.read_consumers(aura_shm::now_ms());
    /// if targets.is_empty() {
...
///
/// ```ignore
/// if let Some(hub) = aura_shm::relay_hub() {
///     if let Some(slot) = hub.claim_slot(aura_shm::now_ms()) {
///         // Use the slot
///     }
/// }
/// ```
